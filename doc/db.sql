...
INSERT INTO `sys_menu` (`menu_id`, `parent_id`, `name`, `url`, `perms`, `type`, `icon`, `order_num`) VALUES ('34', '31', '参数化文件管理', 'modules/test/stressTestParamFile.html', 'test:stress', '1', 'fa fa-file-text-o', '3');
INSERT INTO `sys_menu` (`menu_id`, `parent_id`, `name`, `url`, `perms`, `type`, `icon`, `order_num`) VALUES ('35', '31', '性能测试报告管理', 'modules/test/stressTestReports.html', 'test:stress', '1', 'fa fa-area-chart', '4');
INSERT INTO `sys_menu` (`menu_id`, `parent_id`, `name`, `url`, `perms`, `type`, `icon`, `order_num`) VALUES ('36', '31', '调试报告管理', 'modules/test/debugTestReports.html', 'test:debug', '1', 'fa fa-area-chart', '5');
INSERT INTO `sys_menu` (`menu_id`, `parent_id`, `name`, `url`, `perms`, `type`, `icon`, `order_num`) VALUES ('37', '31', '分布式节点管理', 'modules/test/stressTestSlave.html', 'test:stress', '1', 'fa fa-cloud', '6');
...
CREATE TABLE `test_stress_case` (
  ...
  `file_type` tinyint(20) NOT NULL DEFAULT 0 COMMENT '文件类型(0:jmx文件，1：参数化文件)',
  `related_id` bigint(20) DEFAULT NULL COMMENT '关联的master机上文件id',
  PRIMARY KEY (`case_id`),
...
CREATE TABLE `test_stress_case_file` (
  ...
  `debug_status` tinyint NOT NULL DEFAULT 0 COMMENT '状态  0：关闭debug  1：开始debug调试模式',a
  `ramp_up` int NOT NULL DEFAULT 10 COMMENT '启动时间，单位秒，脚本执行多久启动完所有县城',
  `num_thread` int NOT NULL DEFAULT 10 COMMENT '单机线程数，每台slave机器启动多少个线程',
  `duration` int NOT NULL DEFAULT 3600 COMMENT '期间，执行时间，单位秒，脚本执行多久停止，0代表永远执行',
  `add_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `add_by` bigint(20) COMMENT '提交用户id',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  `update_by` bigint(20) COMMENT '修改用户id',
  `file_type` tinyint NOT NULL DEFAULT 0 COMMENT '文件类型。0：jmx脚本，1：数据文件',
  `related_id` bigint(20) COMMENT '关联的master机器上文件id',
  `dubbo_case` tinyint NOT NULL DEFAULT 0 COMMENT '是否dubbo测试脚本。0：否，1：是',
  PRIMARY KEY (`file_id`),
...
INSERT INTO `sys_config` (`id`, `key`, `value`, `status`, `remark`) VALUES ('8', 'INFLUX_DB_HOST_PORT', 'localhost:8086', '1', 'influxDB配置');