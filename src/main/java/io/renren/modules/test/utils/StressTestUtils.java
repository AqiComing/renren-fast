...
//            .recordStats() // 开启缓存统计
    ...
    public final String getInfluxDbHostPort() {
        return sysConfigService.getValue("INFLUX_DB_HOST_PORT");
    }
    ...
    public void updateRemoteHosts(List<StressTestSlaveEntity> slaveList) {
        String remoteHosts = slaveList.stream().map(slave -> slave.getIp() + ":" + slave.getJmeterPort()).collect(Collectors.joining(","));
        Map hostMap = new HashMap();
        hostMap.put("remote_hosts", remoteHosts);
        PropertiesUtils propertiesUtils = new PropertiesUtils();
        propertiesUtils.updateProperties(getJmeterHomeBin() + File.separator + "jmeter.properties", hostMap);
    }
}