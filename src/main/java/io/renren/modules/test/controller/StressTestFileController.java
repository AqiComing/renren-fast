...
public class StressTestFileController {
    ...
    public R run(@RequestBody Long fileId, @RequestHeader("token") String token) {
        return R.ok(stressTestFileService.run(fileId, token));
    }
    ...
    public ResponseEntity<InputStreamResource> downloadFile(@PathVariable("fileId") Long fileId) {
        ...
        String filePath = stressTestFileService.getFilePath(stressTestFile);
        if (stressTestFile.getFileType() == 0) {
            fileResource = new FileSystemResource(stressTestFileService.removeBackEndListener(filePath));
        } else {
            fileResource = new FileSystemResource(filePath);
        }
