...
//        new JavassistEngine().fixJmeterStandrdEngine();
    ...
    @Autowired
    private StressTestParamFileService testParamFileService;

    @Autowired
    private StressTestSlaveService testSlaveService;
    ...
    public void save(MultipartFile multipartFile, String filePath, StressTestEntity stressCase, StressTestFileEntity stressTestFile) {
        ...
        if (stressTestFile.getFileType() == 0) {
            initJmxConfig(filePath, stressTestFile);
        }
    ...
    public void update(StressTestFileEntity stressTestFile) {
        ...
        try {
            ...
            replaceWithJmxTree(filePath, jmxTree);
        } catch (IOException e) {
            throw new RRException("编辑文件失败！", e);
        }
    ...
    public void updateStatusBatch(StressTestFileEntity stressTestFile) {
        ...
        map.put("rampUp", stressTestFile.getRampUp(10));
...
//            FileUtils.deleteQuietly(new File(jmxDir));
    ...
    @Override
    @Transactional
    public String run(Long fileId, String token) {
        StringBuilder sb = new StringBuilder();
        sb.append(runSingle(fileId, token));
        return sb.toString();
    }
     ...
     * 同理，也不会回滚这一事务。
     */
    public String runSingle(Long fileId, String token) {
        ...
        if (StringUtils.isEmpty(slaveStr)) {
            throw new RRException("没有启用的slave机器。");
        }
...
//            Long caseId = stressTestFile.getCaseId();
//            List<StressTestFileEntity> files = testParamFileService.queryParamFileList(caseId);
//            if (CollectionUtils.isNotEmpty(files)) {
//                while (st.hasMoreElements()) {
//                    String currentSlaveIp = (String) st.nextElement();
//                    StressTestSlaveEntity slave = testSlaveService.queryByIp(currentSlaveIp);
//                    files.stream().forEach(file -> putFileToSlaveByApi(slave, file, token));
//                }
//            }
//
//        }

        if (stressTestUtils.isUseJmeterScript() || stressTestFile.isDubboCase()) {
...
//                Object engine = engineRun(stressTestFile, jmxTree);
            ...
            for (HashTree item : jmxTree.values()) {
                ...
                for (Object key : treeKeys) {
                    if (key instanceof ThreadGroup) {
                        ...
                        ((ThreadGroup) key).setProperty(ThreadGroup.RAMP_TIME, stressTestFile.getRampUp(10));
                        ((ThreadGroup) key).setProperty(ThreadGroup.NUM_THREADS, stressTestFile.getNumThread(10));
    ...
    public void checkHashTree(Iterator<HashTree> values, StressTestFileEntity stressTestFile) {
        List<Object> removeList = new ArrayList<>();
        while (values.hasNext()) {
            HashTree currentNode = values.next();
            Iterator<Object> keySet = currentNode.keySet().iterator();
            while (keySet.hasNext()) {
                Object currentKey = keySet.next();

                //移除原有的backend listener
                if (currentKey instanceof BackendListener) {
                    keySet.remove();
                    removeList.add(currentKey);
                }
                if (currentKey instanceof ThreadGroup) {
                    //如果线程组里有任何一个是dubbo请求则标记为dubbo用例
                    if (!stressTestFile.isDubboCase()) {
                        if (currentNode.keySet().stream().anyMatch(obj -> obj instanceof  DubboSample)) {
                            stressTestFile.setDubboCase(true);
                        }
                    }
                    //统一设置默认循环时间
                    setScheduleInfo((ThreadGroup) currentKey, stressTestFile, 5, 5, 20);

                    checkHashTree(currentNode.values().iterator(), stressTestFile);
                }
            }
            removeList.stream().forEach(object -> currentNode.remove(object));
        }
    }

    public void removeBackendListener(Iterator<HashTree> values) {
        List<Object> removeList = new ArrayList<>();
        while (values.hasNext()) {
            HashTree currentNode = values.next();
            Iterator<Object> keySet = currentNode.keySet().iterator();
    ...
    @Override
    public File removeBackEndListener(String filePath) {
        File jmxFile = new File(filePath);
        String newFilePath = filePath.split(".jmx") + "_download.jmx";
        ...
        try {
            ...
            removeBackendListener(jmxTree.values().iterator());
            ...
            SaveService.saveTree(jmxTree, output);
        } catch (IOException e) {
            throw new RRException("编辑文件失败！", e);
        } finally {
            if (output != null) {
...
//        }
    ...
    public void initJmxConfig(String filePath, StressTestFileEntity stressTestFile) {
        File jmxFile = new File(filePath);
        try {
            stressTestUtils.setJmeterProperties();
            HashTree jmxTree = SaveService.loadTree(jmxFile);
            ...
            Iterator<HashTree> root = jmxTree.values().iterator();

            checkHashTree(root, stressTestFile);

            //dubbo接口添加默认的backend listener组件用于实时监控
//            if (stressTestFile.isDubboCase()) {
//                addBackendListener(jmxTree);
//            }
            addBackendListener(jmxTree);
            ...
            throw new RRException("获取脚本配置信息失败！", e);
        }
    }

    public void addBackendListener(HashTree jmxTree) {
        ...
        InfluxdbClient influxDbListener = new InfluxdbClient();
        Arguments args = influxDbListener.getDefaultParameters();
        Map<String, String> argMap= args.getArgumentsAsMap();
        argMap.put("application","JmeterReport");
        argMap.put("summaryOnly","false");
        argMap.put("influxdbUrl","http://" + stressTestUtils.getInfluxDbHostPort() + "/write?db=renren_fast");
        List<Argument> argList  = argMap.keySet().stream().map(key -> new Argument(key, argMap.get(key))).collect(Collectors.toList());
        args.setArguments(argList);

        listener.setArguments(args);
        listener.setClassname(InfluxdbClient.class.getName());

        jmxTree.add(jmxTree.getArray()[0], listener);
    }

    public void setScheduleInfo(ThreadGroup threadGroup, StressTestFileEntity stressTestFile, int rampUp, int numThread, int duration) {
        //统一设置默认循环时间
        threadGroup.setScheduler(true);
        threadGroup.setRampUp(rampUp);
        stressTestFile.setRampUp(rampUp);
        threadGroup.setNumThreads(numThread);
        stressTestFile.setNumThread(numThread);
        threadGroup.setDuration(duration);
        stressTestFile.setDuration(duration);
    }

    public void replaceWithJmxTree(String filePath, HashTree jmxTree)  {
        File jmxFile = new File(filePath);
        OutputStream output = null;
        jmxFile.delete();
        ...
        if (!jmxFile.exists()) {
            ...
            throw new RRException("原始文件删除失败，无法生成新文件！");
        }
    }
